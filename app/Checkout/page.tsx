...
export default function Checkout() {
    ...
    const handleCashOrder = async (values: checkoutSchemaType) => {
        ...
        try {
            ...
            if (res.status === "success") {
                ...
                setTimeout(() => router.push('/allorders'), 3000);
            } else {
    ...
    const handleCardPayment = async (values: checkoutSchemaType) => {
        ...
        try {
            // The API appends /allorders automatically, so we just pass the origin
            const baseUrl = window.location.origin;
            const res = await createCheckoutSession(cartData._id, baseUrl, values);
            if (res.status === 'success') {
